...
nycflights %>% group_by(month) %>% summarise(iqr_dd = IQR(dep_delay), mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), median_dd = median(dep_delay), n = n())


nycflights %>% group_by(carrier) %>% summarise(iqr_dd = IQR(dep_delay), mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), median_dd = median(dep_delay), n = n())



nycflights <- nycflights %>% mutate(dep_type = ifelse(dep_delay < 5, "on time", "delayed"))

nycflights  %>% group_by(tailnum)  %>% summarise(avg_speed = max(distance / (air_time / 60))) %>%   arrange(desc(avg_speed))

nycflights <- nycflights %>% mutate(avg_speed = distance / (air_time/60))
ggplot(data = nycflights, aes(x=distance, y = avg_speed)) + geom_point()

ggplot(data = nycflights, aes(x = dep_type, fill = arr_typ)) + geom_bar() 
